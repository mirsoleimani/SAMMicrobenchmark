                                        ...
                                        unsigned int iter )
{
    ...
    if( (iter > 0) && (iter % haloWidth) == 0 )
    {
        ...
        if( this->HaveNorthNeighbor() )
        {
            ...
            cudaMemcpy( flatData + (haloWidth * nCols),   // dest
                        currBuf + (haloWidth * nCols),     // src
        ...
        if( this->HaveSouthNeighbor() )
        {
            ...
            cudaMemcpy( flatData + ((nRows - 2*haloWidth)*nCols),   // dest
                        currBuf + ((nRows - 2*haloWidth)*nCols),    // src
                        nsDataSize,                 // amount to transfer
                        cudaMemcpyDeviceToHost );   // direction
        }
        ...
        if( this->HaveEastNeighbor() )
        {
            ...
            cudaMemcpy2D( flatData + (nCols - 2*haloWidth), // dest
                            nCols * sizeof(T),              // dest pitch
                            currBuf + (nCols - 2*haloWidth),    // src
                            nCols * sizeof(T),              // src pitch
        ...
        if( this->HaveSouthNeighbor() )
        {
            ...
            cudaMemcpy( currBuf + ((nRows - haloWidth)*nCols),    // dest
                        flatData + ((nRows - haloWidth)*nCols),   // src
                        nsDataSize,                 // amount to transfer
                        cudaMemcpyHostToDevice );   // direction
        }
        ...
        cudaMemcpy2D( altBuf + (mtx.GetNumRows() - 1) * mtx.GetNumColumns(),      // destination
                        rowExtent,  // destination pitch
                        currBuf + (mtx.GetNumRows() - 1) * mtx.GetNumColumns(),   // source
                        rowExtent,  // source pitch
                        rowExtent,  // width of data to transfer (bytes)
                        1,          // height of data to transfer (rows)
                        cudaMemcpyDeviceToDevice );
