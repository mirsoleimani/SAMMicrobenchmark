                                        ...
                                        cl::CommandQueue& queue )
{
    ...
    if( (iter > 0) && (iter % haloWidth) == 0 )
    {
        ...
        size_t nsDataItemCount = haloWidth * nCols;
        ...
        if( this->HaveNorthNeighbor() )
        {
            ...
            queue.enqueueReadBuffer( currBuf,
                                        CL_FALSE,    // blocking
                                        haloWidth * nCols * sizeof(T),          // offset
                                        nsDataSize, // size
                                        flatData + (haloWidth * nCols),
                                        NULL,
                                        &nEvent );
        ...
        if( this->HaveSouthNeighbor() )
        {
            ...
            queue.enqueueReadBuffer( currBuf,
                                        CL_FALSE,    // blocking
                                        (nRows - 2 * haloWidth) * nCols * sizeof(T),    //offset
                                        nsDataSize,
                                        flatData + ((nRows - 2*haloWidth) * nCols),
                                        NULL,
                                        &sEvent );
        ...
        if( this->HaveEastNeighbor() )
        {
            for( unsigned int r = 0; r < nRows; r++ )
            {
                ...
                {
                    // east side
                    flatData[r*nCols + (nCols - 2*haloWidth) + hc] = eData[r*haloWidth + hc];
                }
            }
        }
        if( this->HaveWestNeighbor() )
        {
            for( unsigned int r = 0; r < nRows; r++ )
            {
                ...
                {
                    // west side
                    flatData[r*nCols + haloWidth + hc] = wData[r*haloWidth + hc];
                }
            }
        }
        ...
        if( this->HaveSouthNeighbor() )
        {
            queue.enqueueWriteBuffer( currBuf,
                                            CL_FALSE,    // blocking
                                            (nRows - haloWidth) * nCols * sizeof(T),    //offset
                                            nsDataSize,
                                            flatData + ((nRows - haloWidth) * nCols),
                                            NULL,
                                            &sEvent );
            waitEvents.push_back( sEvent );
        }
        ...
        if( this->HaveEastNeighbor() )
        {
            ...
            for( unsigned int r = 0; r < nRows; r++ )
            {
                ...
                {
                    // east side
                    eData[r*haloWidth + hc] = flatData[r*nCols + (nCols - haloWidth) + hc];
                }
            }
        ...
        if( this->HaveWestNeighbor() )
        {
            ...
            for( unsigned int r = 0; r < nRows; r++ )
            {
                ...
                {
                    // west side
                    wData[r*haloWidth + hc] = flatData[r*nCols + hc];
                }
            }
        ...
        if( this->HaveEastNeighbor() )
        {
            eEvent = copyRectFunc( altBuf,         // dest
                            ...
                            (int)mtx.GetNumColumns(),    // dest pitch
                            currBuf,      // src
                            (int)(mtx.GetNumColumns() - 1),              // src offset
                            (int)mtx.GetNumColumns(),    // src pitch
                            1,              // width
