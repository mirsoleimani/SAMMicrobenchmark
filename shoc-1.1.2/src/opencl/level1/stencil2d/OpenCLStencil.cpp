...
OpenCLStencil<T>::operator()( Matrix2D<T>& mtx, unsigned int nIters )
{
    ...
    // in host code we presume first dimension is rows, second is cols,
    // but kernel coordinate system has first dim x (corresponds to cols)
    // and second dim y (corresponds to rows)
    cl::KernelFunctor func = kernel.bind( queue,
        cl::NDRange( mtx.GetNumColumns() - 2, mtx.GetNumRows() - 2 ),
        cl::NDRange( lCols, lRows ) );
